-- JAMMIN Command Center: DJ training live database installer
...
select
  (select count(*) from public.training_categories) as training_categories,
  to_regclass('public.staff_training') is not null as staff_training_ready,
  to_regclass('public.training_history') is not null as training_history_ready,
  to_regclass('public.training_completion_notifications') is not null as completion_email_log_ready;
